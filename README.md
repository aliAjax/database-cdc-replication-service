...
- `txapply` applies complete source transactions through a journaled commit state machine.
- `slotlease` renews replication-slot ownership and fences stale capture workers.
- `schemabarrier` coordinates schema publication with version-aware event routing.
- `snapshandoff` publishes snapshot manifests through an archive-backed handoff.
